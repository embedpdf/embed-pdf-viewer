 ...
 * via `pnpm test`, which respects `dependsOn: ["^build"]` from `turbo.json`.
 *
 * The browser-mode project is path-filtered in CI; locally, run
 * `pnpm --filter @embedpdf/angular-pdf-viewer test:browser` (or invoke
 * `vitest --project angular-browser` from the root).
 */
