...
/** Everything that ships to npm at 3.0. Angular is validated by ng-packagr +
 * its parity script; pdf-runtime's wasm build has its own verify script —
 * both are still PACKED here (they're in the dependency closure), just not
 * imported by fixtures directly (angular needs an Angular toolchain). */
const PACK_DIRS = [
  ...fs
    .readdirSync(path.join(repoRoot, 'v3', 'packages'))
    .filter((d) => d !== 'angular')
    .map((d) => path.join('v3', 'packages', d)),
  'packages/engine',
  'packages/engine-core',
  'packages/engine-services',
  'packages/pdf-runtime',
  // per-target sidecars (optionalDependencies of pdf-runtime): wasm32 and the
  // current platform's napi target are REQUIRED (bundlers resolve the wasm32
  // import statically; node loads the local napi binary). The other targets
  // are CI cross-compiles — packed when present, skipped with a log otherwise
  // (missing ones resolve as failed OPTIONAL deps in fixtures, which is
  // exactly what a real single-platform consumer install looks like).
  ...sidecarDirs(),
];

function sidecarDirs(): string[] {
  const npmDir = path.join(repoRoot, 'packages', 'pdf-runtime', 'npm');
  ...
  for (const d of fs.readdirSync(npmDir)) {
    ...
    if (built) dirs.push(path.join('packages', 'pdf-runtime', 'npm', d));
    else if (required.has(d)) {
      console.error(`✖ required pdf-runtime target not built: npm/${d}`);
      process.exit(1);
...
if (missing.length) {
  ...
  console.error(
    '  Run: pnpm turbo run build --filter="./v3/packages/*" --filter="./packages/engine*" --filter="./packages/pdf-runtime"',
  );
  process.exit(1);
}
