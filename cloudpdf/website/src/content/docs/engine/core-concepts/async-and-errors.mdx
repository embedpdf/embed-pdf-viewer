...
const pending = doc.page(pageRef).render.image({ viewport: { kind: 'width', width: 1600 } });
...
try {
  const text = await doc.page(pageRef).text.read();
} catch (err) {
...
```ts
const pending = doc.page(pageRef).render.image({ viewport: { kind: 'width', width: 1200 } });
