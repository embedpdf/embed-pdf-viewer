...
<Callout type="warn">
  Signing rides <code>doc.sign</code>; a certification additionally needs{' '}
  <code>doc.sign.certify</code>. A locked field, a certification that forbids the edit, or a
  required seed-value entry the engine does not implement all answer <code>SignatureRefused</code>{' '}
