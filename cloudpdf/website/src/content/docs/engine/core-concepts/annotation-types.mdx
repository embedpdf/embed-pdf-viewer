...
- `print`, `hidden`, `locked` and the other PDF flags — each its own field, e.g. `print: true` (see [Annotations](/docs/engine/core-concepts/annotations#flags)).
...
A `stamp` places an image — or a vector appearance — inside `rect`. The data describes the
stamp; its drawing travels beside it, as the `appearance` resource in the second argument.
There is no separate upload step. The format is detected from the bytes themselves (never
from a declared mime type), and each format takes the best path into the PDF:
...
await page.annotations.create(
  ...
  { appearance: file }, // PNG, JPEG, or single-page PDF bytes
);
```
...
| :-------- | :------------------------------------------------------------------ | :------- | :---------- |
| `rect`    | Bounding box                                                        | yes      | —           |
| `fit`     | How the drawing fills `rect` (see below)                            | no       | `'contain'` |
| `name`    | Standard stamp label (`'Approved'`, `'Draft'`, `'Confidential'`, …) | no       | none        |
| `opacity` | Transparency `0`–`1`, applied once over the whole drawing           | no       | `1`         |

`fit` uses the CSS `object-fit` vocabulary: `'contain'` preserves the aspect ratio and
keeps the drawing fully visible, `'cover'` preserves the aspect ratio and fills the box
(may crop), `'fill'` stretches to the box. The stamp records it, so reads return it and a
later resize fills the new box the same way. A stamp made by another tool records none and
reads `fit: null`; it is re-fit with `'fill'`, the way every PDF viewer places an
appearance in its box.
...
Stamps with the same artwork share one drawing in the file, each with its own box, fit,
rotation and opacity around it. Placing a stamp a hundred times stores its image once, and
so does placing it again after the document is reopened. Changing one of those stamps never
changes the others.

Changing the geometry later (an `update` with a new `rect`, or a resize in the viewer)
re-fits the existing drawing — the image is never re-uploaded or re-encoded. A new `fit`
re-fits it too. A new `appearance` resource replaces the drawing:

```ts
await page.annotations.update(ref, { fit: 'cover' });
await page.annotations.update(ref, {}, { appearance: otherImage });
```

A stamp created without an `appearance`, or with bytes that aren't PNG, JPEG or PDF, is
refused with `InvalidArg` before any work happens.

`opacity` is the stamp's `/CA`. A new value replaces the old one, and it applies to the
drawing as a whole, so overlapping parts of the artwork don't show through each other.
Opacity goes both ways with Acrobat: a stamp from Acrobat keeps the opacity it was saved
with through a resize or a copy, and a new value replaces Acrobat's; a stamp made here
can have its opacity changed in Acrobat, and Acrobat's value replaces ours. Neither
stacks on top of the other.

### Copying a stamp

A read never contains the drawing. `readResource(ref, 'appearance')` returns it as a
one-page PDF, before the `fit`, rotation and `opacity` the data describes. The read and
that PDF together make the same stamp again, in this document or another:
...
A stamp made by another tool is returned as its drawing at its own size, as its appearance
defines it. Its data reads `fit: null`, so a copy stretches the drawing to its `rect`, the way
every PDF viewer shows it, and looks the same. The same artwork at two sizes is one drawing. A
stamp made from a PNG or JPEG is returned as the image at its own size, a pixel to a point,
whatever box it was placed in, so a later `fit` always has the whole image to work with.

The same drawing is always the same bytes, whichever engine exports it, and a drawing
exported again after a copy is the same bytes as before. You can store drawings by their hash
and keep one copy for every stamp that uses it. Reading a resource sends document content out,
so it needs the `doc.download` scope.

## File attachment

A `file-attachment` shows an icon on the page and carries a file. The data holds the file's
name, MIME type and description; the file's bytes travel beside it, as the `file` resource.
...
await page.annotations.create(
  {
    ...
    icon: 'paperclip',
    file: { name: 'drawing.pdf', mimeType: 'application/pdf', description: 'Rev B' },
  },
  { file: pdfBytes },
);
```

A read returns the same `file`, plus `size` and `checksum` taken from the bytes, and
`createdAt` and `modifiedAt` from the embedded file's parameters; the bytes themselves come
from `readResource(ref, 'file')`. In an update, `file` replaces the name, MIME type and
description together (leave one out and it is removed), and a `file` resource replaces the
bytes:

```ts
const { file } = attachment;
await page.annotations.update(ref, { file: { ...file, name: 'drawing-rev-c.pdf' } });
await page.annotations.update(ref, {}, { file: newBytes });
```
