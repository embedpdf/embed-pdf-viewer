...
A read returns the same `file`, plus `size`, `checksum` and `creationDate` taken from the
bytes; the bytes themselves come from `downloadFile(ref)`. In an update, `file` replaces the
