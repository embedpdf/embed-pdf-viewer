...
Pages are addressed by their durable **`PageRef`** (`{ kind: 'objectNumber',
pageObjectNumber }` — the page's indirect object number), never by index.
`pages.list()` returns every page's geometry in display order, so grab the first
one's `ref` and render it.

```ts
const { pages } = await doc.pages.list();
const first = doc.page(pages[0].ref);
