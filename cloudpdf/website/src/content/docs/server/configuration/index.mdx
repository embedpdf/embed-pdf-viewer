...
<Callout type="info">
  ...
  runs with just that. Production keys additionally require the three auth
  secrets below to be real, random values of at least 32 bytes.
</Callout>
...
| `CLOUDPDF_PASSWORD_VERIFICATION_HMAC_SECRET` | dev fallback | HMAC pepper for encrypted-PDF password proofs. **Required (≥32 bytes) under a production license.** |
| `CLOUDPDF_PASSWORD_SESSION_SERVER_SECRET` | dev fallback | Server key for encrypted-PDF password sessions. **Required (≥32 bytes) under a production license.** |
