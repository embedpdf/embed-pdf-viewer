...
describe('digital signatures over the wire (native runtime)', () => {
  test('prepare (multipart) → complete (JSON) publishes a version; the catalog and immutable reads follow', async () => {
    ...
    const before = await json<{
      signatures: Array<{ fieldName: string; signed: boolean }>;
      revisions: unknown[];
    }>(await call('GET', `${layer}/signatures`, docId, 'alice'));
    expect(before.signatures).toEqual([
      expect.objectContaining({ fieldName: 'sig', signed: false }),
    ]);

    const preparedRes = await call(
      'POST',
    ...
    const pendingManifest = await json<{
      layerVersion: number;
      working: boolean;
      docVersion: number;
    ...
    const stale = await call(
      ...
      JSON.stringify({
        cms: toBase64(cms),
        expectedVersion: { ...prepared.expectedVersion, editsVersion: 99 },
      }),
      'application/json',
    );
    ...
    expect(completed.signature).toMatchObject({
      signed: true,
    ...
    expect(versions.versions).toEqual([
      ...
      expect.objectContaining({
        number: 2,
        sha256: completed.version.sha256,
        parentSha256: seeded.sha,
        producer: 'signature',
        signingId: prepared.signingId,
      }),
    ]);
    const head = await json<{ baseSha: string; docVersion: number }>(
      await call('GET', `/v1/docs/${docId}/head`, docId, 'alice'),
    );
    ...
    const vsig = await call(
      'GET',
      `/v1/docs/${docId}/versions/signatures/${completed.version.sha256}`,
      docId,
      'alice',
    );
    expect(vsig.headers.get('cache-control')).toContain('immutable');
    const vsnap = await json<{
      revisions: Array<{ end: number }>;
      signatures: Array<{ fieldName: string; contentsSize: number }>;
    }>(vsig);
    expect(vsnap.revisions).toHaveLength(2);
    const contents = await call(
      'GET',
      `/v1/docs/${docId}/versions/signatures/${completed.version.sha256}/${encodeFieldKey('sig')}/contents`,
      docId,
      'alice',
    );
    ...
    const digest = await call(
      'GET',
      `/v1/docs/${docId}/versions/signatures/${completed.version.sha256}/${encodeFieldKey('sig')}/digest/sha256`,
      docId,
      'alice',
    );
    ...
    const download = await call(
      'GET',
      `/v1/docs/${docId}/versions/download/${completed.version.sha256}`,
      docId,
      'alice',
    );
    ...
    const rev0 = await call(
      'GET',
      `/v1/docs/${docId}/versions/revisions/${completed.version.sha256}/0`,
      docId,
      'alice',
    );
    ...
    expect(
      (
        await call(
          'GET',
          `/v1/docs/${docId}/versions/revisions/${completed.version.sha256}/7`,
          docId,
          'alice',
        )
      ).status,
    ...
    expect(
      (await call('GET', `/v1/docs/${docId}/versions/download/${'0'.repeat(64)}`, docId, 'alice'))
        .status,
    ).toBe(404);
    ...
    expect(manifest).toMatchObject({
      baseSha: completed.version.sha256,
      baseByteLength: completed.version.byteLength,
      layerVersion: 3,
      working: false,
    });
    ...
    const form = await json<{ fields: Array<{ name: string; value: unknown }> }>(
      await call('GET', `${layer}/form`, docId, 'alice'),
    );
    expect(JSON.stringify(form.fields.find((f) => f.name === 'group.total')?.value)).toContain(
      'wire',
    );

    // Replay with the same CMS: already-completed; a different CMS: refused; cancel: already-completed.
    ...
    expect(replay).toMatchObject({
      ...
      version: { sha256: completed.version.sha256 },
    });
    const other = await call(
      ...
      'alice',
      JSON.stringify({
        cms: toBase64(new Uint8Array([0x30, 3, 2, 1, 1])),
    ...
    expect(
      await json<{ status: string }>(
        await call('DELETE', `${layer}/signatures/${prepared.signingId}`, docId, 'alice'),
      ),
  ...
  test('analysis URLs: the policy version is a served cache key, and only settled verdicts are immutable', async () => {
    ...
    const ok = await call(
      'GET',
      versionUrl(`since.signature=0&policy=${SIGNATURE_POLICY_VERSION}`),
      docId,
      'alice',
    );
    ...
    const stale = await call(
      'GET',
      versionUrl(`since.signature=0&policy=${SIGNATURE_POLICY_VERSION + 1}`),
      docId,
      'alice',
    );
    ...
    const exploratory = await call(
      'GET',
      versionUrl(`since.signature=0&level=fill&policy=${SIGNATURE_POLICY_VERSION}`),
      docId,
      'alice',
    );
    ...
    const manifest = await json<{ docVersion: number }>(
      await call('GET', `${layer}/manifest`, docId, 'alice'),
    );
    const tokenFor = (policyVersion: number) =>
      encodeAnalysisToken({
        docVersion: manifest.docVersion,
        since: { signatureIndex: 0 },
        policyVersion,
      });
    const layerOk = await call(
      'GET',
      `${layer}/signatures/analysis@${tokenFor(SIGNATURE_POLICY_VERSION)}`,
      docId,
      'alice',
    );
    expect(layerOk.headers.get('cache-control')).toContain('immutable');
    expect((await json<{ policyVersion: number }>(layerOk)).policyVersion).toBe(
      SIGNATURE_POLICY_VERSION,
    );
    const layerStale = await call(
      'GET',
      `${layer}/signatures/analysis@${tokenFor(SIGNATURE_POLICY_VERSION + 1)}`,
      docId,
      'alice',
    );
  ...
  test('cancel frees the layer; expiry is swept; a sibling behind the head cannot sign', async () => {
    ...
    const p1 = decodePrepared(
      SignaturePreparedWireSchema.parse(
        await json(
          await call(
            'POST',
            `${alice}/signatures/prepare`,
            docId,
            'alice',
            prepareForm({ field: { kind: 'fqn', name: 'sig' } }),
          ),
    ...
    expect(
      await json<{ status: string }>(
        await call('DELETE', `${alice}/signatures/${p1.signingId}`, docId, 'alice'),
      ),
    ...
    expect(
      (
        await call(
          'POST',
          `${alice}/signatures/${p1.signingId}/complete`,
          docId,
          'alice',
          JSON.stringify({
            cms: toBase64(new Uint8Array([0x30, 3, 2, 1, 1])),
            expectedVersion: p1.expectedVersion,
          }),
          'application/json',
        )
      ).status,
    ...
    const cms = await buildDetachedCms({
      digest: p3.digest,
      hash: p3.algorithm,
      profile: profileFor(p3.subFilter as SignatureSubFilter),
      signer,
    });
    const done = await json<{ status: string; version: { sha256: string } }>(
      await call(
        'POST',
        `${alice}/signatures/${p3.signingId}/complete`,
        docId,
        'alice',
        JSON.stringify({ cms: toBase64(cms), expectedVersion: p3.expectedVersion }),
        'application/json',
      ),
    );
    expect(done.status).toBe('completed');

    const bobManifest = await json<{ baseSha: string; scopes: Record<string, string> }>(
      await call('GET', `${bob}/manifest`, docId, 'bob'),
    );
    ...
    const staleBob = await call(
      'POST',
      `${bob}/signatures/prepare`,
      docId,
      'bob',
      prepareForm({ field: { kind: 'fqn', name: 'sig' } }),
    );
    ...
    const bobForm = await json<{ fields: Array<{ name: string; value: unknown }> }>(
      await call('GET', `${bob}/form`, docId, 'bob'),
    );
    expect(JSON.stringify(bobForm.fields.find((f) => f.name === 'group.total')?.value)).toContain(
      'bob',
    );

    // A fresh layer after the publish inherits everything over version 2.
    const carol = await json<{
      baseSha: string;
      scopes: Record<string, string>;
      docVersion: number;
