...
const acroForm = (fields: number[]) => `<< /Fields [${fields.map((n) => `${n} 0 R`).join(' ')}] /DA (/Helv 0 Tf 0 g) >>`;
...
function document(objects: Objects, catalogExtra = ''): Uint8Array {
  return pdf({ 1: `<< ${CATALOG}${catalogExtra} >>`, 2: '<< /Type /Pages /Kids [3 0 R] /Count 1 >>', ...objects });
}
...
describe.skipIf(!OUT)('interop fixtures for the revision analysis (pyHanko)', () => {
  ...
  const open = (bytes: Uint8Array): Promise<Doc> => engine.open({ kind: 'bytes', id: `interop-${++seq}`, bytes }, { scope: ['*'] });

  async function certify(bytes: Uint8Array, permission: 1 | 2 | 3, fieldName = 'sig'): Promise<Uint8Array> {
  ...
  const scenarios: Scenario[] = [
    {
      ...
      build: async () => {
        const base = document(
          { 3: page([30]), 9: '<< /S /JavaScript /JS (var example = 1;) >>', 10: acroForm([30]), 30: sigField(30, 'sig', '10 10 100 40'), 50: '<< /Certs [] >>' },
    ...
    },
    {
      ...
      build: async () => {
        ...
        return append(await certify(base, 1), { 9: '<< /S /JavaScript /JS (var example = 2;) >>', 50: '<< /Certs [] /VRI <<>> >>' });
      },
    },
    {
      ...
      expected: 'permitted',
      build: async () => {
        const base = document({ 3: page([20, 30]), 10: acroForm([20, 30]), 20: textField('a', ' /AP 22 0 R'), 22: '<< /N 23 0 R >>', 23: formStream('q Q'), 30: sigField(30, 'sig', '10 10 100 40') });
        return append(await certify(base, 2), { 20: textField('b', ' /AP 22 0 R'), 23: formStream('q 0.5 g Q') });
      },
    },
    {
    ...
    },
    {
      ...
      expected: 'forbidden',
      build: async () => {
        const other = (v: string) => `<< /Type /Annot /Subtype /Widget /FT /Tx /T (other) /V (${v}) /Rect [20 90 180 110] /P 3 0 R /AP 22 0 R >>`;
        const base = document({
          3: page([20, 40, 30]),
    ...
    },
    {
      ...
      build: async () => {
        const base = document(
          { 3: page([30]), 9: '<< /S /JavaScript /JS (var example = 1;) >>', 10: acroForm([30]), 30: sigField(30, 'sig', '10 10 100 40') },
        ...
        return append(signed, { 1: root.replace(/\/OpenAction\s*9 0 R/, '/OpenAction << /S /JavaScript /JS (var example = 2;) >>') });
      },
    },
    {
      ...
      build: async () => {
        ...
        try {
          await sign(doc, { field: { kind: 'fqn', name: 'sig2' }, signer, lock: { action: 'include', fields: ['text'] } });
          return new Uint8Array(await doc.download());
        } finally {
          await doc.close();
        }
    ...
    },
    {
      ...
      expected: 'permitted',
      build: async () => {
        const base = document({ 3: page([20, 30]), 10: acroForm([20, 30]), 20: textField('a'), 30: sigField(30, 'sig', '10 10 100 40') });
        const certified = await certify(base, 2);
        const doc = await open(certified);
        try {
          await doc.forms.setValue({ kind: 'fqn', name: 'text' }, { type: 'text', value: 'filled' });
          return new Uint8Array(await doc.download());
        } finally {
          await doc.close();
        }
    ...
    },
    {
      ...
      expected: 'unchanged',
      note:
        'input only: pyh_sign_new_field.py appends a VISIBLE signed field in one revision; we judge the output permitted (ISO 32000-2 P=2 lists signing; Acrobat agrees). pyHanko refuses it under its default allow_new_visible_after_certify=False, a policy stricter than the standard.',
      build: async () => certify(document({ 3: page([30]), 10: acroForm([30]), 30: sigField(30, 'sig', '10 10 100 40') }), 2),
    },
  ];
  ...
  test.skipIf(!process.env.EPDF_INTEROP_ANALYZE)('analyze an external file', async () => {
    ...
    try {
      ...
      console.log(
        JSON.stringify({
          verdict: a.verdict,
          steps: a.steps.map((s) => ({ level: s.levelInForce, verdict: s.verdict, objects: s.changes.map((c) => c.objectNumber), findings: s.findings })),
        }),
      );
    } finally {
      await doc.close();
  ...
  test.skipIf(!process.env.EPDF_INTEROP_NATIVE)('file-backed signing on the native runtime', async () => {
    ...
    try {
      ...
      await writeFile(basePath, document({
      ...
      const doc = await native.open({ kind: 'layerFile', id: 'native-file', basePath }, { scope: ['*'] });
      let sealedPath: string;
      try {
        await doc.forms.setValue({ kind: 'fqn', name: 'text' }, { type: 'text', value: 'filled on disk' });
        const result = await sign(doc, { field: { kind: 'fqn', name: 'sig' }, signer, certify: { permission: 2 } });
        expect(result.status).toBe('completed');
        // The session's base is now the sealed candidate file beside the base.
        const { readdir } = await import('node:fs/promises');
        const files = (await readdir(OUT!)).filter((f) => f.startsWith('native_base.pdf.signing-'));
  ...
  test('write fixtures and manifest', async () => {
    ...
    for (const s of scenarios) {
      ...
      manifest.push({ name: s.name, expected: s.expected, ours, ...(s.note ? { note: s.note } : {}) });
