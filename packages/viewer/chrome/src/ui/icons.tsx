/**
 * Icon set — Tabler-style stroke icons as path data, rendered by one <Icon>
 ...
 * command's resolved `iconAccent`. With no accent, a slotted stroke inherits
...
export const ICON_PATHS: Record<string, readonly PathSpec[]> = {
  ...
  highlight: [
    // the marker block behind the "A" — visible only when accented
    {
  ...
  // A dashed frame + a small rounded hatched box. This icon system is plain
  // paths (no <clipPath>), so the hatch segments are pre-clipped to the
  // 10..22 box analytically.
  redactArea: [
  ...
  // the ungroup glyph: two separated rounded rects
  ungroup: [
...
export function Icon({ name, size = 20, strokeWidth = 2, className, title, accent }: IconProps) {
  ...
  return (
    ...
    >
      {paths.map((path, i) => {
        if (typeof path === 'string') return <path key={i} d={path} />;
        const fill =
          path.fill === true ? 'currentColor' : path.fill ? accent?.[path.fill] : undefined;
        // a stroke slot paints from the accent, inheriting currentColor
        // without one; a fill-only path never gets an outline
        const stroke = path.stroke ? accent?.[path.stroke] : path.fill ? 'none' : undefined;
        return (
          <path
            key={i}
            d={path.d}
