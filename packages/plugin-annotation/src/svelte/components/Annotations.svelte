...
<script lang="ts">
  ...
  const pointerHandlers = usePointerHandlers({ pageIndex });
  ...
  $effect(() => {
    ...
    const off = annotationCapability.provides.onStateChange((state) => {
  ...
  const handlers: PointerEventHandlersWithLifecycle = {
    onPointerDown: (_: unknown, pe: PointerEvent) => {
      if (pe.target === pe.currentTarget && annotationCapability.provides) {
        annotationCapability.provides.deselectAnnotation();
        editingId = null;
      }
    },
  };
  ...
  $effect(() => {
    return pointerHandlers.register(handlers);
  });

  // click/select logic shared across shapes
  function handleClick(e: MouseEvent | TouchEvent, annotation: TrackedAnnotation) {
    e.stopPropagation();
    if (annotationCapability.provides && selectionCapability.provides) {
      annotationCapability.provides.selectAnnotation(pageIndex, annotation.object.id);
...
{#each annotations as annotation (annotation.object.id)}
  ...
  {@const tool = annotationCapability.provides?.findToolForAnnotation(annotation.object)}
