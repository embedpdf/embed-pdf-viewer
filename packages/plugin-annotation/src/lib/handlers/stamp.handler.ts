    ...
    return {
      onPointerDown: (pos) => {
        ...
        const placeStamp = (imageData: ImageData, width: number, height: number) => {
          // Center the stamp at the click position (click point), then clamp to stay fully on-page.
          const finalClickX = clamp(pos.x, width / 2, pageSize.width - width / 2);
          const finalClickY = clamp(pos.y, height / 2, pageSize.height - height / 2);

          const finalX = finalClickX - width / 2;
          const finalY = finalClickY - height / 2;
