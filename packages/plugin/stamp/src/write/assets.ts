...
export function createAssetWrites(
  ...
  const createAsset = async (input: AddAssetInput): Promise<string> => {
    ...
    if (name.length === 0 || name.includes('=')) {
      throw stampError(
        'invalid-input',
        `invalid stamp identifier '${name}': must be non-empty and contain no '='`,
      );
    }
    const rasterSize =
    ...
    if (!isPdf && (!rasterSize || rasterSize.width <= 0 || rasterSize.height <= 0)) {
      throw stampError(
        'invalid-input',
        'a raster asset needs a positive `size` (its page size in points)',
      );
    }
  ...
  return {
    api: {
      createAsset: verb(createAsset),
      createAssetFromAnnotations: verb(createAssetFromAnnotations),
      updateAsset: verb(updateAsset),
      deleteAsset: verb(deleteAsset),
      moveAsset: verb(moveAsset),
      duplicateAsset: verb(duplicateAsset),
