...
import { createRedactionCapability } from '../src/controller';
import type { RedactionAction, RedactionState } from '../src/model';
...
describe('marking the selection', () => {
  const makeCtx = (opts: { selection?: unknown; canCreate?: boolean } = {}) => {
    ...
    const ctx = {
      ...
      cleanup: () => {},
      document: () => null,
    } as unknown as ControllerContext<RedactionState, RedactionAction>;
