...
/** What the effects need from the context — the controller context satisfies it. */
export type DocumentEventsContext = Pick<
  PluginContext<AnnotationState, AnnotationAction>,
  'doc' | 'get' | 'getState' | 'dispatch' | 'cleanup' | 'document'
>;

export function subscribeDocumentEvents(ctx: DocumentEventsContext): void {
  const doc = ctx.doc;
  if (!doc) return;
