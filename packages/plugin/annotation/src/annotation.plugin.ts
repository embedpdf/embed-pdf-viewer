...
export const annotationPlugin = (config: AnnotationConfig = {}) =>
  definePlugin<AnnotationState, AnnotationAction, AnnotationHostCapability>({
    ...
    // so it needs the config too — not just the reducer state. `connect` runs
    // once every capability exists: it folds in confirmed document events and
    // registers this plugin's tools and pointer handlers with the hub.
    create: (ctx) => ({
      api: createAnnotationController(ctx, config),
      connect() {
...
function wireHub(ctx: {
  get<T>(token: import('@embedpdf/core').CapabilityToken<T>): T;
  tryGet<T>(token: import('@embedpdf/core').CapabilityToken<T>): T | null;
  ...
  for (const tool of annotation.listResolvedTools()) {
    ...
    // A sibling that added its tools through `registerTool` (forms) already
    // announced them to the hub while this plugin was still connecting.
