...
export function createStamps(
  ctx: Pick<AnnotationContext, 'doc' | 'dispatch' | 'getState' | 'tryGet'>,
  {
    store,
    geometry,
    records,
  ...
  const placeStamp = async (
    ...
    const placed = createStampAt(
      ...
      placement.rotation ?? 0,
      {
        ...(input.name !== undefined ? { name: input.name } : {}),
        ...(input.subject !== undefined ? { subject: input.subject } : {}),
        opacity: defaultsFor(store.model(), ARMED_STAMP_TOOL_ID).opacity,
      },
    );
    if (!placed) {
      throw new Error(
  ...
  const placeArmedStamp = (pon: number, point: Vec, displayRotation?: number): boolean => {
    ...
    return stageStampAt(
      ...
      tools.uprightRotFor(displayRotation),
      {
        ...
        opacity: defaultsFor(store.model(), tool?.preset ?? ARMED_STAMP_TOOL_ID).opacity,
      },
    );
  };
  ...
  const placeStampSource = async (
    ...
    stageStampAt(pon, point, source, desiredStampSize(meta, targetWidth), rotCW, { opacity });
  };
  ...
  const requestStampAt = (pon: number, point: Vec, displayRotation?: number): boolean => {
    ...
    const opacity = defaultsFor(store.model(), tool.preset).opacity;
    if (spec.kind === 'bytes') {
  ...
  const api = {
    ...
    placeArmedStamp: (page: PageRef, point: Vec, displayRotation?: number) =>
      placeArmedStamp(page.pageObjectNumber, point, displayRotation),
    requestStampAt: (page: PageRef, point: Vec, displayRotation?: number) =>
      requestStampAt(page.pageObjectNumber, point, displayRotation),
    getArmedStampPreview: (devicePixelWidth?: number) => {
      const payload = armed;
    ...
    getStampArmEpoch: () => ctx.getState().stampArmEpoch,
  };
