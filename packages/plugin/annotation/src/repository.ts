...
export function fromDTO(
  ...
  return {
    ...
    // links (grouped children of another kind) fold onto their PARENT's `link`
    // slot instead, in `foldAttachedLinks`.
    ...(dto.subtype === 'link' ? { link: dto.target } : {}),
...
/* ── attached links: fold + desired-state derivation ──────────────────────── */
...
function baseCreateDraft(a: Annot, crop: PdfRect): AnnotationDraft | null {
  ...
  // link: the invisible hit rectangle + its own `/A` target (goto/uri, or
  // null for the create-then-edit flow). No style — links paint nothing.
  if (a.subtype === 'link' && f && 'rect' in f)
    return { subtype: 'link', rect: f.rect, target: writableTarget(a.link) };
  return null;
}
...
export function toPatch(a: Annot, crop: PdfRect): AnnotationPatch | null {
  ...
  // link: geometry + target. Three-state target: a writable value replaces
  // `/A`; an explicit model `null` CLEARS it (the engine removes /A + /Dest);
  // a read-only arm (javascript/named/goto-remote/…) is left untouched —
  // geometry-only patch, the foreign action survives every move.
  if (a.subtype === 'link' && f && 'rect' in f) {
    const target = writableTarget(a.link);
    return {
      subtype: 'link',
      rect: f.rect,
      ...(target ? { target } : a.link == null ? { target: null } : {}),
    };
  }
  return null;
}
