...
/**
 * Set the document's /Trapped Info entry from the wire enum. The DTO only
 * carries `true | false | unknown`, so there is no "clear" form here; omit
 * `trapped` from the patch to leave it untouched.
 */
