...
describe('matchLiteral', () => {
  test('finds a plain match', () => {
    expect(find('Hello World', { text: 'World' })).toEqual([{ start: 6, count: 5 }]);
  });
  ...
  test('ignores diacritics by default, both directions', () => {
    expect(find('Le café était', { text: 'cafe' })).toEqual([{ start: 3, count: 4 }]);
    expect(find('the cafe', { text: 'café' })).toEqual([{ start: 4, count: 4 }]);
  });
  ...
  test('matches across line wraps (whitespace collapse)', () => {
    ...
    expect(find('hello\n   world', { text: 'hello world' })).toEqual([{ start: 0, count: 14 }]);
  });
  ...
  test('finds ligature text with a plain-letters needle', () => {
    expect(find('ﬁle system', { text: 'file' })).toEqual([{ start: 0, count: 3 }]);
  });

  test('multiple non-overlapping matches, advancing past each', () => {
    expect(find('aaa', { text: 'aa' })).toEqual([{ start: 0, count: 2 }]);
  ...
  test('wholeWord accepts hits at text edges and punctuation boundaries', () => {
    expect(find('cat', { text: 'cat', wholeWord: true })).toEqual([{ start: 0, count: 3 }]);
    expect(find('a cat, dog', { text: 'cat', wholeWord: true })).toEqual([{ start: 2, count: 3 }]);
  });
