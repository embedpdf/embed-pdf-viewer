...
const directory = fixturePath('p3-matched');
...
const reports: any[] = [];
afterAll(async () => {
  await writeFile(reportPath('p3-matched/engine-report.json'), JSON.stringify(reports, null, 2));
});
for (const runtime of ['wasm', 'native'] as const) {
  describe(runtime, () => {
    ...
    test.each(manifest.cases)(
      '$id preserves certification integrity and reports the expected identities',
      async (fixture: any) => {
        const path = `${directory}/${fixture.file}`;
        const bytes = new Uint8Array(await readFile(path));
        const doc = await engine.open(
          runtime === 'native'
            ? { kind: 'layerFile', id: fixture.id, basePath: path }
            : { kind: 'bytes', id: fixture.id, bytes },
          { scope: ['*'] },
        );
        try {
          ...
          reports.push({
            ...
            docMdp: results[0].signature.docMdp,
            catalogCertification: results[0].signature.catalogCertification,
            allowsAnnotationModification: doc.security.allows('doc.annotate.modify'),
            summary: results[0].summary,
            integrity: results[0].integrity,
            cryptography: results[0].cryptography,
            trust: results[0].trust,
            modifications: results[0].modifications,
            weakCount,
            annotationRefs: squares.map((a) => ({ contents: a.contents, ref: a.ref })),
          ...
          expect(squares.map((a) => a.contents)).toEqual(
            deleted ? ['weak-0', 'weak-2', 'weak-3'] : ['weak-0', 'weak-1', 'weak-2', 'weak-3'],
          );
