...
for (const runtime of ['wasm', 'native'] as const) {
  test(`unchanged page rewrite: ${runtime}`, async () => {
    const path = fixturePath('supplemental/approval-inline-rewrite-size-preserved.pdf');
    ...
    try {
      ...
      await writeFile(
        reportPath(`supplemental/size-engine-${runtime}.json`),
