...
const directory = fixturePath('pdfs');
...
const reports: any[] = [];
afterAll(async () => {
  await writeFile(reportPath('engine-report.json'), JSON.stringify(reports, null, 2));
});
  ...
  describe(runtime, () => {
    ...
    test.each(manifest.cases)('$id', async (fixture: any) => {
      ...
      const doc = await engine.open(
        runtime === 'native'
          ? { kind: 'layerFile', id: fixture.id, basePath: `${directory}/${fixture.file}` }
      ...
      try {
        ...
        const expected =
          fixture.action === 'control'
            ? 'unchanged'
            : ['p2', 'lockp2'].includes(fixture.profile)
              ? 'forbidden'
              : 'permitted';
    ...
    test.each(['p2', 'lockp2'])('engine refuses protected weak deletion: %s', async (profile) => {
      ...
      const doc = await engine.open(
        { kind: 'bytes', id: `guard-${runtime}-${profile}`, bytes },
        { scope: ['*'] },
      );
      try {
        const page = (await doc.pages.list()).pages[0].ref;
