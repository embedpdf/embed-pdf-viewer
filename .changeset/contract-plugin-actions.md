...
- Internal layout: `contract` / `host-contract` / `model` / `controller` with `services`, `submit`, `scripting`, `dispatch` and `lifecycle` areas. The `/internal` entry is removed; hosts use `/contract/host`.
