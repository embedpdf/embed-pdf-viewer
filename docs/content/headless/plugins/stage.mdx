...
`goToPage`, `next`, `previous`, and `reveal`.
...
- **`next()` / `previous()` respect spreads.** When a two-page spread fits, they
...
`getScrollMetrics()` (on the host lens, `@embedpdf/plugin-stage/contract/host`)
returns `scrollTop`, `scrollHeight`, and `clientHeight` (the same numbers a DOM
element would report), and `scrollTo()` /
...
```ts
stage.rotateViewBy(90); // one quarter-turn clockwise from here
...
<Cards>
  ...
  <Card
    ...
    href="/docs/engine/documents/rendering"
  />
  <Card
    title="All plugins"
