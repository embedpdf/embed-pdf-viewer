...
`renderPlugin()` to draw page bitmaps. Put a `<Stage>` in your tree and pass
a render function for each visible page. Here, just the rendered page:
...
- **A page you own.** `{() => <RenderLayer />}` runs once per visible page.
  ...
  highlights) and they position themselves with the page.
