...
signaturePlugin({
  // The key. A thunk resolves per signing (prompt a PIN, fetch a token…).
  key: () =>
  ...
  // 'sign' (default with a key) | 'visual' (default without) | 'ask'
  mode: 'sign',
});
```
...
await signature.placeMark({ assetId }, { page: page.ref, at: { x: 120, y: 90 } });
```
...
```ts
// Seal: the mark becomes the widget's appearance, the key signs the digest.
const result = await signature.sign({
  field: { kind: 'fqn', name: 'sig' },
  mark: { assetId },
  signer: { reason: 'Approved', location: 'Amsterdam' }, // the name defaults to the certificate's subject
...
`sign` runs the engine's two-phase signing (`prepare` → your key →
`complete`) through `@embedpdf/core-signature`; any failure cancels the
candidate, so a document is never left with a pending signing. A call can
bring its own `key`; the configured one is used otherwise. A signed
