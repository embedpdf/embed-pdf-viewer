...
Two limits are by design: the browser's native undo stack does not span a
formatting change applied through the plugin, and the DOM's line wrapping
approximates the engine's — the appearance stream the engine bakes is the
truth, and it is what every other viewer shows.
