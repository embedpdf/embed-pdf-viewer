...
[Free text](/docs/engine/annotations/free-text#rich-text)
...
- **The plugin decides** (`@embedpdf/plugin-annotation`, DOM-free). Its
  `textItems(page)` projection hands the framework one editable element per
...
`Helvetica, Arial, sans-serif`, and so on).

<Engine local>

The full viewer does the fetch, the registration and the mount from one
`annotations.fonts` option — see
[Fonts](/docs/engine/documents/fonts#show-the-font-in-the-browser-too).
