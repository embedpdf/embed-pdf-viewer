...
- **A signed document is protected.** What its signatures forbid (a certification's DocMDP level,
  field locks) is subtracted from what the caller may do, exactly like encryption permission bits.
...
snapshot.protection; // { level, certification, fieldLocks, policyVersion }
```
...
for (const v of verdicts) {
  ...
  v.modifications; // { verdict } — what changed after it, from analyze()
...
Once a document carries signatures, the engine enforces what they forbid by default: a certified
document loses page assembly, a `P=1` certification loses form fill, a FieldMDP-locked field refuses
writes. The refusals surface as `ProtectedDocument`. Read the policy back from `snapshot.protection`
to grey out what a user cannot do before they try. An engine constructed with `signedDocumentPolicy: 'permit'` turns
enforcement off for tools whose job is to produce or test invalid files; analysis never depends on it.
