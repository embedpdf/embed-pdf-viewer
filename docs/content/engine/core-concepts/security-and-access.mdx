...
`identity` is who the session acts for, or `null` when anonymous. It is one
object on both engines: the local engine takes it as the `identity` open option,
the cloud engine from the document token's `identity` claim.

```ts
interface Identity {
  userId?: string;             // owner of the annotations this session creates
  displayName?: string;        // their author (/T)
  email?: string;
  title?: string;
  organization?: string;
  organizationalUnit?: string;
  groupId?: string;            // the group new annotations are created in
  groups?: string[];           // groups the caller belongs to, for :group= scopes
}
```

Only `displayName`, `userId` and `groupId` are written into annotations. The
other fields are there for scripts (`identity.corporation`, `identity.email`)
and your own UI.
