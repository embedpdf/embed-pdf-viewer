...
A stamp made by another tool is returned as it is shown, on a page the size of its `rect`.
Reading a resource sends document content out, so it needs the `doc.download` scope.
