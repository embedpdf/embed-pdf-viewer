...
reads `fit: null`; it is re-fit with `'contain'`.
...
from `downloadFile(ref)`. In an update, `file` replaces the
name, MIME type and description together (leave one out and it is removed), and a `file`
resource replaces the bytes:
