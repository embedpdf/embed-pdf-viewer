...
A stamp made by another tool is returned as it is shown, in the size of its `rect`. A stamp
made from a PNG or JPEG is returned as the image at its own size, a pixel to a point, whatever
box it was placed in, so a later `fit` always has the whole image to work with.
