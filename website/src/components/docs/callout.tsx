...
const styles: Record<CalloutType, { box: string; icon: string; body: string }> = {
  info: {
    ...
    icon: 'bg-[#DCEAFF] text-ep-blue',
    body: 'text-[#2A4574] [&_b]:text-ep-navy [&_strong]:text-ep-navy',
  },
  warn: {
...
export function Callout({ type = 'info', children }: { type?: CalloutType; children: ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`font-sans text-[15px] leading-[1.6] ${s.body} [&_a]:text-ep-blue [&>:first-child]:mt-0 [&_a:hover]:underline [&_a]:font-semibold`}
      >
        {children}
      </div>
    </div>
