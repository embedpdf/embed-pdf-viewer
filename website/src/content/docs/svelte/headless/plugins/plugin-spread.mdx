...
The plugin provides the `useSpread` store to control the layout mode. When you change the mode, the plugin recalculates the page groupings, and the `<Scroller>` component automatically updates to reflect the new layout.

### Building a Spread Mode Toolbar

Use the `useSpread` store to get the current `spreadMode` and the `provides` object for changing it for a specific document. You can then build a UI, like a set of buttons, to allow users to switch between layouts.
...
import { SpreadExample } from '../../code-examples/headless/spread-example';

<CodeExample 
  codePaths={[
    "../../examples/svelte-tailwind/src/examples/headless/spread-example.svelte",
    "../../examples/svelte-tailwind/src/examples/headless/spread-example-content.svelte"
  ]}
>
  <SpreadExample />
</CodeExample>
...
### Store: `useSpread(documentId)`

Connects your component to the spread plugin's state and functions for a specific document.
