...
The plugin works by registering a "pan" interaction mode. You can build UI to toggle this mode on and off using the `usePan` store.

### Building a Pan Tool Button

The `usePan` store provides the `isPanning` state and a `provides` object with methods to control the pan mode for a specific document. The `isPanning` boolean is perfect for providing visual feedback in your UI, such as highlighting the tool's button when it is active.
...
<CodeExample 
  codePaths={[
    "../../examples/svelte-tailwind/src/examples/headless/pan-example.svelte",
    "../../examples/svelte-tailwind/src/examples/headless/pan-example-content.svelte"
  ]}
>
  <PanExample />
</CodeExample>
