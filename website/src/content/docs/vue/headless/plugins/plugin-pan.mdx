...
The `usePan` composable provides the `isPanning` state and a `provides` object with methods to control the pan mode for a specific document. The `isPanning` boolean is perfect for providing visual feedback in your UI, such as highlighting the tool's button when it is active.
...
You should wrap your `<Viewport>` component with `<GlobalPointerProvider>` and provide the `documentId`.
...
<template>
  <GlobalPointerProvider :document-id="activeDocumentId">
    <Viewport :document-id="activeDocumentId">
      <Scroller :document-id="activeDocumentId">{/* ... */}</Scroller>
    </Viewport>
...
import { PanExample } from '../../code-examples/headless/pan-example';

<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/pan-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/pan-example-content.vue"
  ]}
>
  <PanExample />
</CodeExample>
...
### Composable: `usePan(documentId)`

Connects your component to the pan plugin's state and functions for a specific document.
