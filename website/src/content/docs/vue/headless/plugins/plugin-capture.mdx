  ...
  <Scroller :document-id="activeDocumentId">
    <template #default="{ page }">
      <PagePointerProvider :document-id="activeDocumentId" :page-index="page.pageIndex">
        ...
        <MarqueeCapture :document-id="activeDocumentId" :page-index="page.pageIndex" />
      </PagePointerProvider>
    </template>
  </Scroller>
</template>
...
const isActive = computed(() => state.value.isMarqueeCaptureActive);
</script>
...
<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/capture-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/capture-example-content.vue"
  ]}
>
  <CaptureExample />
</CodeExample>
...
| :--- | :--- | :--- |
| **`state`** | `Ref<CaptureState>` | A ref object containing the current state, such as `isMarqueeCaptureActive`. |
| **`provides`**| `Ref<CaptureScope \| null>` | A ref object with methods to interact with the plugin, or `null` if not ready. |
