    ...
    <ThumbnailsPane :document-id="documentId">
      <template #default="{ meta }">
        <div
          ...
          @click="scroll?.scrollToPage({ pageNumber: meta.pageIndex + 1 })"
        >
          <div :style="{ width: `${meta.width}px`, height: `${meta.height}px` }">
...
import { ThumbnailExample } from '../../code-examples/headless/thumbnail-example';

<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/thumbnail-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/thumbnail-example-content.vue"
  ]}
>
  <ThumbnailExample />
</CodeExample>
...
| **`meta`** | `ThumbMeta` | **(Required)** The metadata object from the `ThumbnailsPane` scoped slot. |
