...
Use the `useSpread` composable to get the current `spreadMode` and the `provides` object for changing it for a specific document. You can then build a UI, like a set of buttons, to allow users to switch between layouts.
...
<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/spread-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/spread-example-content.vue"
  ]}
>
  <SpreadExample />
</CodeExample>
...
### Composable: `useSpread(documentId)`
