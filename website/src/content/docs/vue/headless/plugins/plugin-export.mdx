...
Use the `useExport` composable to get access to the plugin's `download` method for the specific document. Call this method from a button in your toolbar to start the download process.
...
<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/export-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/export-example-content.vue"
  ]}
>
  <ExportExample />
</CodeExample>
...
### Composable: `useExport(documentId)`

This composable connects your component to the export plugin's functions for a specific document.
...
| :--- | :--- | :--- |
| **`provides`** | `Ref<ExportScope \| null>` | A ref object with methods to control exporting, or `null` if the plugin is not ready. |
