...
Use the `usePrint` composable to get access to the plugin's `print` method for the current document. You can call this method from any component, such as a button in your toolbar.
...
const handlePrint = () => {
  ...
  const printTask = print.value.print(); // Prints the entire document
...
<CodeExample 
  codePaths={[
    "../../examples/vue-tailwind/src/examples/headless/print-example.vue",
    "../../examples/vue-tailwind/src/examples/headless/print-example-content.vue"
  ]}
>
  <PrintExample />
</CodeExample>
...
### Composable: `usePrint(documentId)`

This composable connects your component to the print plugin's functions for a specific document.
...
| :--- | :--- | :--- |
| **`provides`** | `Ref<PrintScope \| null>` | A ref object with methods to control printing, or `null` if the plugin is not ready. |
