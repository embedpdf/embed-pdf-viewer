  ...
  <EmbedPDF :engine="engine" :plugins="plugins" v-slot="{ activeDocumentId }">
    ...
    >
      <template v-if="isLoaded">
        <Toolbar :document-id="activeDocumentId" />
...
For example, the `<Toolbar.vue>` component uses multiple composables to manage different functionalities like panning, rotating, and changing the page layout. Since EmbedPDF supports multiple documents, composables require a `documentId` parameter (as a getter function).

```vue filename="src/components/Toolbar.vue"
<script setup lang="ts">
...
// Pass documentId as a getter function to each composable
const { provides: panProvider, state: panState } = usePan(() => props.documentId);
const { provides: rotateProvider } = useRotate(() => props.documentId);
const { state: spreadState, provides: spreadProvider } = useSpread(() => props.documentId);
// ...
</script>
...
<template>
  <v-app-bar>
    <v-toolbar>
      <!-- ... -->
      <v-btn 
        :color="panState.isPanning ? 'primary' : undefined"
        @click="panProvider?.togglePan()"
