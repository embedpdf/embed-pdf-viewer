...
- a field the engine can only read, such as a geospatial measure (`geospatial`) or a link's
  JavaScript action (`unsupported-action`): the annotation is imported without it;
