...
The first argument is data only. The kinds that carry bytes take them in a second
argument, by what the bytes are for: a stamp's drawing as `appearance`, an attached file as
`file`. `update(ref, patch, resources)` takes them the same way, to replace them. A read
never contains bytes; `readResource(ref, role)` returns them, so a read and its resources
make the same annotation again.
...
`doc.annotations.export` takes annotations out of a document with the bytes beside them, a stamp's
drawing or an attached file, as one **bundle**, in one call:

```ts
const bundle = await doc.annotations.export(); // every annotation
const some = await doc.annotations.export({ refs: [comment.ref] }); // one comment and its thread
const pages = await doc.annotations.export({ pages: [pageRef] }); // everything on a page
```

A bundle is a flat list. Each item is an annotation exactly as a read returns it, and names its
resources by id. Each resource is in the bundle once, however many items use it, under the SHA-256
of its bytes: a hundred stamps of one image carry the image once.
...
- by default a comment also brings every reply below it (`include: 'threads'`). Pass
  `include: 'references'` to leave the replies out.

The bundle also lists every page its items are on or point at, with the page's position and crop
box, so the annotations can be placed on the matching pages of another document.

To store or send a bundle as one file, `AnnotationTransfer` writes it as JSON with its resources in
base64, and reads it back, checking every resource against its id:

```ts
import { AnnotationTransfer } from '@embedpdf/engine-core';

const text = AnnotationTransfer.stringify(bundle);
const again = await AnnotationTransfer.parse(text); // refuses anything tampered with
```

Export reads the annotations and the bytes beside them, so it needs `doc.annotate.read` and
`doc.download`. A bundle has limits, 50 MB and 10 000 annotations by default; an export that would
pass one is refused with `PayloadTooLarge`, naming the limit, so you can export fewer pages at a
time. The same document exports to the same bundle on the local and the cloud engine; on the cloud,
an export is cached until the document's annotations or pages change.

## Importing annotations

`doc.annotations.import` creates a bundle's annotations in a document, in one call and as one
change:

```ts
const result = await target.annotations.import(bundle, { attribution: 'stamp' });

result.created; // the new annotations, in bundle order
result.refMap; // each annotation's ref in the bundle, and its ref here
result.dropped; // what couldn't be carried, and why
```

Everything is checked before anything is written, and the import is all or nothing: a failure
while writing leaves the document exactly as it was.

By default each annotation goes to the page with the same ref (`pages: 'same'`), which fits a
bundle from the same document or a copy of it. For another document, map pages by their position,
or name the pairs:

```ts
await target.annotations.import(bundle, { attribution: 'stamp', pages: 'by-position' });
await target.annotations.import(bundle, {
  attribution: 'stamp',
  pages: [{ from: sourcePage, to: targetPage }],
});
```

A link's destination maps the same way. A page an annotation is on or points at that maps to no
page refuses the import with `InvalidArg`, naming the pages.

Replies are linked to the comments they answer, and popups to their annotations, among the
imported annotations. What an import can't carry it leaves out, reports in `dropped`, and imports
the rest:

- an annotation of a kind the engine doesn't model (`unsupported-kind`);
- a form field's widget (`form-field`): it travels with its field, through `doc.forms`;
- a field the engine can only read, such as a geospatial measure (`geospatial`), a link's
  JavaScript action, or actions a kind has no field for (`unsupported-action`): the annotation is
  imported without it;
- an annotation whose `nm` is already used on its page (`name-conflict`), so importing the same
  bundle twice doesn't duplicate named annotations;
- an annotation whose comment or popup parent was left out (`parent-dropped`) or isn't in the
  bundle (`parent-missing`).

By default an import restores the attribution the bundle has. Each annotation keeps its author,
its creation and modification dates (in their own time zones) and its owner (`userId`,
`groupId`), and records the importing session's user as `importedBy`. That writes authorship that
isn't the session's, so it needs `doc.annotate.import`: grant it to trusted jobs, such as a sync
from your own database, not to end users.

`attribution: 'stamp'` makes each annotation as `create` would instead: the author, the dates and
the owner are the session's, and it needs only the authority a create needs.

An import emits one `annotation.created` event per annotation, all sharing `origin.tx`. On the
cloud it is one request: its `opId` (minted when you don't pass one) makes a retry safe, as a
second request with the same `opId` returns the first one's result. The limits are the export's;
a bundle past one is refused with `PayloadTooLarge` before anything is written.
