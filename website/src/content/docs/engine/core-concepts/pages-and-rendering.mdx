...
for (const page of pages) {
  console.log(page.index, page.ref, page.label, page.width, page.height, page.rotation);
}
```

Each entry is a `PageLayout`: display `index`, durable `ref`,
...
`ref` is the page's `PageRef` — `{ kind: 'objectNumber', pageObjectNumber }`,
its indirect object number — and the only key to correlate pages across
calls: it survives reorder, rotate and every edit, and a deleted page's
number is retired, never recycled. `index` is display order and shifts on a
move. Every verb below takes refs; `toPageRef(n)` builds one from a number
you hold.
...
await doc.page(page.ref).render.image({ viewport }); // always on the lattice
```
</Engine>
...
`doc.pages.move(pages, destIndex)` detaches the listed pages (a `PageRef[]`) and
re-inserts them as a contiguous block at `destIndex` (in the post-removal index
space), preserving the order you pass them.

```ts
// Move the pages at display index 6 and 8 so they sit starting at index 0.
const { pages } = await doc.pages.list();
await doc.pages.move([pages[6].ref, pages[8].ref], 0);
```
...
```ts
await doc.pages.rotate([page.ref], 90); // 0 | 90 | 180 | 270
```

`delete` removes pages by identity. Deleting every page is rejected — a
document keeps at least one — and a deleted page's object number is retired, never recycled;
...
added.insertedPages; // fresh refs, in insertion order
...
```ts
const bytes = await doc.pages.extract!([pageA.ref, pageB.ref]);
...
layout.namedPages; // [{ name: 'Approved=Approved', target: { kind: 'page', page } }, …]

// Register (or retarget) a name; `replace` drops another key in the same job — a rename.
await doc.pages.setName!({ name: 'Draft=Concept', page: page.ref, replace: 'Draft=Draft' });
