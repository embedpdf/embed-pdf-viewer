...
const SquareHandle = ({ style, backgroundColor, ...rest }: HandleProps) => (
  <div
    {...rest}
    style={{
      ...style,
...
const DiamondVertex = ({ style, backgroundColor, ...rest }: HandleProps) => (
  <div
    {...rest}
    style={{
      ...style,
...
interface RotationHandleUI {
  ...
  color?: string;          // Background color (default: '#007ACC')
  iconColor?: string;      // Icon stroke color (default: 'white')
  margin?: number;         // Gap from bounding box edge in px (default: 20)
  showConnector?: boolean; // Show connector line (default: false)
  connectorColor?: string; // Connector line color (default: same as color)
...
```typescript
type SelectionOutlineStyle = 'solid' | 'dashed' | 'dotted';

interface SelectionOutline {
  color?: string;               // Outline color (default: '#007ACC')
  style?: SelectionOutlineStyle; // Default: 'solid' (single) / 'dashed' (group)
