...
// Renders a single view pane
const MyViewPane = ({ viewId }) => (
  <ViewContext viewId={viewId}>
    {({ documentIds, activeDocumentId, setActiveDocument, isFocused, focus }) => (
      <div 
        className={`view-pane ${isFocused ? 'focused' : ''}`}
        onClick={focus}
      >
        {/* Render Tabs */}
        <div className="tabs">
          {documentIds.map(docId => (
            <button 
              ...
              className={docId === activeDocumentId ? 'active' : ''}
            >
              {docId}
            </button>
          ))}
...
To create a dynamic layout, use the `useAllViews` hook to iterate over available views and render them.
...
export const SplitLayout = () => {
  ...
  const gridClass = views.length > 1 ? 'grid-cols-2' : 'grid-cols-1';

  return (
    <div className={`grid h-full ${gridClass} gap-4`}>
      {views.map((view) => (
        <MyViewPane key={view.id} viewId={view.id} />
      ))}
    </div>
  );
};
...
import { useViewManagerCapability } from '@embedpdf/plugin-view-manager/react';
...
const OpenButton = () => {
  ...
  const handleOpen = async () => {
    ...
    const targetViewId = viewManager.getFocusedViewId() || viewManager.getAllViews()[0].id;

    // 3. Add the document to that view
    viewManager.addDocumentToView(targetViewId, result.documentId);
    
    // 4. Make it visible immediately
    viewManager.setViewActiveDocument(targetViewId, result.documentId);
  };
...
import { PDFViewer } from '../../code-examples/headless/view-manager-example';

<CodeExample 
  codePaths={[
    "content/docs/react/code-examples/headless/view-manager-example.tsx"
  ]}
>
  <PDFViewer />
</CodeExample>
...
A headless provider that exposes data and actions for a specific view via a render prop.
