...
The `usePan` hook provides the `isPanning` state and a `provides` object with methods to control the pan mode for a specific document. The `isPanning` boolean is perfect for providing visual feedback in your UI, such as highlighting the tool's button when it is active.

```tsx
import { usePan } from '@embedpdf/plugin-pan/react';

export const PanToolbar = ({ documentId }) => {
...
import { PDFViewer } from '../../code-examples/headless/pan-example';

<CodeExample 
  codePaths={[
    "content/docs/react/code-examples/headless/pan-example.tsx"
  ]}
>
  <PDFViewer />
</CodeExample>
