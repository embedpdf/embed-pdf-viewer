...
Use the `useSpread` hook to get the current `spreadMode` and the `provides` object for changing it for a specific document. You can then build a UI, like a set of buttons, to allow users to switch between layouts.

```tsx {2, 5}
import { SpreadMode } from '@embedpdf/plugin-spread';
import { useSpread } from '@embedpdf/plugin-spread/react';

export const SpreadToolbar = ({ documentId }) => {
...
import { PDFViewer } from '../../code-examples/headless/spread-example';

<CodeExample 
  codePaths={[
    "content/docs/react/code-examples/headless/spread-example.tsx"
  ]}
>
  <PDFViewer />
</CodeExample>
