...
<Callout type="info">
  Signing needs <code>doc.sign</code> (a certification also <code>doc.sign.certify</code>); a visual
  fill needs <code>doc.forms.fill</code>. <code>canSign()</code>, <code>canFill()</code> and{' '}
  <code>canCertify()</code> mirror those for your buttons.
</Callout>
...
The plugin re-reads and re-validates on `document.versioned` — a signature
completed here or, on the cloud, in another session.
