...
For one page, pass its durable `PageRef` (`page.ref` from `usePageList()`), a
starting character index, and a character count:
...
A search hit is a range in the same character space, so it selects as it is:
...
```ts
selection.select({
  start: { page: firstPage.ref, index: 120 },
  end: { page: lastPage.ref, index: 35 },
});
```
...
selection.segmentsForPage(page.ref);
```
...
| `segmentsForPage(page)` | Get oriented highlight segments for one page            |
| `rectsForPage(page)`    | Get simple bounding boxes for one page                  |
...
`@embedpdf/plugin-selection` or its framework adapter. The `/contract/host`
entry point is for framework and plugin integration and is not a public
