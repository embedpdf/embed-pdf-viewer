...
search.canSearch({ snippets: true }); // …with text snippets? (needs copy too)
...
try {
  ...
  if (isPluginError(err, 'permission-denied')) {
    // err.details.required — the capability that was missing, e.g. 'doc.forms.fill'
  }
}
```

The local and the cloud engine refuse the same way, so this one check covers both.
...
| search     | `canSearch({ snippets? })` — `snippets: true` asks about snippets |
| render     | `canRender()`                                             |
