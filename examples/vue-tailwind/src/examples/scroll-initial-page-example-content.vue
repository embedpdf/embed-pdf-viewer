...
import { Scroller, useScroll, type ScrollPlugin } from '@embedpdf/plugin-scroll/vue';
...
// Scroll to the initial page when the layout is ready
onMounted(() => {
  if (!scrollCapability.value) return;

  unsubscribe = scrollCapability.value.onLayoutReady((event) => {
    // Only scroll on the initial layout (not on tab switches)
    if (event.documentId === props.documentId && event.isInitial) {
      scrollCapability.value?.forDocument(props.documentId).scrollToPage({
        pageNumber: props.initialPage,
